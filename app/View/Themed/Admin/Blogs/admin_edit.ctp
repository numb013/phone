...
//echo pr($item_genres);
     ...
     ?>
    <h1>Add Page</h1>
    <?php echo $this->Form->create('Blog', array('type' => 'file', 'url' => 'edit')); ?>
    ...
    <script type="text/javascript">
      ...
      function addPhoto(){
        ...
        $(imagID).on('change', function(){ // 解説①
          ...
          if(strFileInfo && strFileInfo.type.match('image.*')){ // 解説③
            ...
            var imgpreview = '<img id="preview" />'.replace("preview", preview);
            $(previewid).remove();  // 解説④
     ...
     $('#idimage0').on('change', function(){ // 解説①
       ...
       if(strFileInfo && strFileInfo.type.match('image.*')){ // 解説③
         ...
         $('#preview').remove();  // 解説④
         $('#preview_area').append('<img id="preview" />'); // 解説⑤
